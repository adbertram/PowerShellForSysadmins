...
function New-PowerLabWebServer {
	[CmdletBinding()]
	param
	(
		[Parameter(Mandatory)]
	...
	Install-PowerLabWebServer -ComputerName $Name -AnswerFilePath -DomainCredential $DomainCredential
}
...
function Install-PowerLabWebServer {
	...
	try {
		...
		## Test to see if IIS  is already enabled
		if (Invoke-Command -Session $session -ScriptBlock { (Get-WindowsFeature -Name 'Web-Server').Installed }) {
			Write-Verbose -Message 'SQL Server is already installed'
		} else {
			Invoke-Command -Session $session -ScriptBlock { Add-WindowsFeature -Name 'Web-Server' }
		}
		$session | Remove-PSSession
	} catch {
		$PSCmdlet.ThrowTerminatingError($_)
